...
async function testScenario1(page){
  //Scenario 1: Intentar iniciar sesion en la pagina, correo y pw aleatorios, assert
  //#region GIVEN
  ...
  //#region WHEN
  var item = getRandomItem(mockData);
...
async function testScenario3(page){
  ...
  await page.fill('css=#user-website', item.website);
  await page.fill('css=#user-bio', item.words);
  ...
  //#region THEN
  await expect.expect(page.locator('input#user-website')).toHaveValue(item.website)
  await expect.expect(page.locator('input#user-email')).toHaveValue(item.email)
  console.log("Expect ok")
  //#endregion
...
async function testScenario4(page){
  ...
  //Fill new user email new-user-email
  await page.type('css=#new-user-email', item.email);
  await page.screenshot({path:`${screenshotPath}-3-fillnewUserEmail.png`})
  ...
  //#region THEN
  await expect.expect(page.getByText(item.email)).toBeHidden();
  console.log('Expect ok')
  //#endregion
...
async function testScenario5(page){
  ...
  //Fill new tag fields
  var item = getRandomItem(mockData);
  await page.fill('input#tag-name', item.companyName);
  await page.fill('input#tag-slug', `${item.companyName}-slug`);
  await page.getByPlaceholder("abcdef").fill(item.hexColor.replace('#',''));
  await page.fill('textarea#tag-description', item.words);
  await new Promise(r => setTimeout(r, 1000));
  ...
  //#region THEN
  await expect.expect(page.getByRole('heading', { name: `Tags ${item.companyName}` })).toBeVisible();
  console.log('Expect ok')
  //#endregion
...
async function testScenario6(page){
  ...
  //Edit new tag
  var editItem = getRandomItem(mockData);
  await page.fill('input#tag-name', editItem.companyName);
  await page.fill('input#tag-slug', `${editItem.companyName}-slug`);
  await page.getByPlaceholder("abcdef").fill(editItem.hexColor.replace('#',''));
  await page.fill('textarea#tag-description', editItem.words);
  await new Promise(r => setTimeout(r, 1000));
  ...
  //#region THEN
  await expect.expect(page.locator('input#tag-name')).toHaveValue(editItem.companyName)
  console.log('Expect ok')
  //#endregion
...
async function testScenario7(page){
  ...
  //#region THEN
  await expect.expect(page.getByText(item.companyName)).toBeHidden();
  console.log('Expect ok')
  //#endregion
...
async function testScenario9(page){
  ...
  var item = getRandomItem(mockData)
  await page.getByPlaceholder("Post Title").fill(item.companyName);
  await page.fill('css=.koenig-editor__editor.__mobiledoc-editor.__has-no-content', item.words);
  await new Promise(r => setTimeout(r, 1000));
  ...
  //#region THEN
  await expect.expect(page.getByText(item.companyName)).toBeVisible();
  console.log('Expect ok')
  //#endregion

  //Open draft and delete it (cleanup)
  await page.getByText(item.companyName).click()
...
async function testScenario10(page){
  ...
  //Fill new post fields
  var item = getRandomItem(mockData)
  var newPostTitle = `${item.companyName} draft`
